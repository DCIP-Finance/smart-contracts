...
contract DCIP is Context, IBEP20, Ownable {
    ...
    constructor (address payable routerAddress, address marketingWalletAddress, address communityInvestWalletAddress) public {
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        if ( block.timestamp < _holderToTimestamp[_msgSender()] + 24 hours){
            return _amount.mul(_taxFee + 1).div(
                10**2
            );    
        }
        else{
    ...
    function calculateBurnFee(uint256 _amount) private view returns (uint256){
        if ( block.timestamp < _holderToTimestamp[_msgSender()] + 24 hours){
            return _amount.mul(4).div(
            10**2
            );
        }
        else{
            return _amount.mul(2).div(
            10**2
            );   
        }
    }
    ...
    function _transfer(
        ...
        if (takeFee){
            ...
            uint256 feeTotal = toMarketingWallet.add(toCommunityWallet).add(toLiquidity).add(toBurn).add(tFee);
            //transfer amount, it will take tax, burn, liquidity fee
            amount = amount.sub(feeTotal);
            
            _tokenTransfer(from,to,amount, false);
            _tokenTransfer(from, _marketingWalletAddress, toMarketingWallet, false);
            _tokenTransfer(from, _communityInvestWalletAddress, toCommunityWallet, false);
            _burnToken(toBurn);
